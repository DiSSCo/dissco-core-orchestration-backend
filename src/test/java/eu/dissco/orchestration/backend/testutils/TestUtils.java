...
public class TestUtils {
	...
	public static final List<String> SS_FILTERS = List
		.of("{<like path='/DataSets/DataSet/Metadata/Description/Representation/Title'>Collection Crustacea SMF</like>}");
	...
	public static JsonApiWrapper givenSourceSystemSingleJsonApiWrapper(int version) {
		...
		return new JsonApiWrapper(
				new JsonApiData(sourceSystem.getId(), ObjectType.SOURCE_SYSTEM, flattenSourceSystem(sourceSystem)),
	...
	public static SourceSystem givenSourceSystem(String id, int version, SourceSystem.OdsTranslatorType translatorType,
	...
	public static SourceSystem givenTombstoneSourceSystem() {
		return givenSourceSystem().withOdsStatus(OdsStatus.TOMBSTONE)
	...
	public static SourceSystemRequest givenSourceSystemRequest(OdsTranslatorType translatorType) {
		return new SourceSystemRequest().withSchemaName(OBJECT_NAME)
	...
	public static JsonApiListWrapper givenMasResponse(List<MachineAnnotationService> machineAnnotationServices,
			JsonApiLinks linksNode) {
		var dataNode = machineAnnotationServices.stream()
			.map(mas -> new JsonApiData(mas.getId(), ObjectType.MAS, flattenMas(mas)))
			.toList();
		return new JsonApiListWrapper(dataNode, linksNode);
	}
	...
	public static DataMapping givenDataMapping(String id, int version, String name) {
		return new DataMapping().withId(id)
			.withSchemaIdentifier(id)
			...
			.withSchemaDateCreated(Date.from(CREATED))
			.withSchemaDateModified(Date.from(CREATED))
			.withOdsHasDefaultMapping(List
				.of(new DefaultMapping().withAdditionalProperty("ods:organisationID", "https://ror.org/05xg72x27")))
			.withOdsHasTermMapping(
					List.of(new TermMapping().withAdditionalProperty("ods:physicalSpecimenID", "dwc:catalogNumber")))
			.withSchemaCreator(givenAgent())
			.withOdsMappingDataStandard(DataMapping.OdsMappingDataStandard.DW_C);
	}

	public static DataMapping givenTombstoneDataMapping() {
		return givenDataMapping().withOdsStatus(DataMapping.OdsStatus.TOMBSTONE)
			.withSchemaDateModified(Date.from(UPDATED))
			.withSchemaVersion(2)
	...
	public static DataMappingRequest givenDataMappingRequest() {
		return new DataMappingRequest().withSchemaName(OBJECT_NAME)
			.withSchemaDescription(OBJECT_DESCRIPTION)
			.withOdsHasDefaultMapping(List
				.of(new DefaultMapping().withAdditionalProperty("ods:organisationID", "https://ror.org/05xg72x27")))
			.withOdsHasTermMapping(
	...
	public static JsonApiListWrapper givenMappingResponse(List<DataMapping> dataMappings, JsonApiLinks linksNode) {
		List<JsonApiData> dataNode = new ArrayList<>();
		dataMappings
			.forEach(m -> dataNode.add(new JsonApiData(m.getId(), ObjectType.DATA_MAPPING, flattenDataMapping(m))));
		return new JsonApiListWrapper(dataNode, linksNode);
	}
	...
	public static MachineAnnotationServiceRequest givenMasRequest() {
		return new MachineAnnotationServiceRequest().withSchemaName(MAS_NAME)
			.withOdsContainerImage("public.ecr.aws/dissco/fancy-mas")
			.withOdsContainerTag("sha-54289")
			.withOdsHasTargetDigitalObjectFilter(
					new OdsHasTargetDigitalObjectFilter().withAdditionalProperty("ods:topicDiscipline", "botany"))
	...
	public static List<EnvironmentalVariable> givenMasEnvironment() {
		return List.of(new EnvironmentalVariable().withSchemaName("server.port").withSchemaValue(8080));
	}

	public static List<SecretVariable> givenMasSecrets() {
		return List
			.of(new SecretVariable().withSchemaName("spring.datasource.password").withOdsSecretKeyRef("db-password"));
	}
	...
	public static MachineAnnotationService givenMas(String id, int version, String name, Integer ttl) {
		return new MachineAnnotationService().withId(id)
			...
			.withOdsContainerTag("sha-54289")
			.withOdsHasTargetDigitalObjectFilter(
					new OdsHasTargetDigitalObjectFilter__1().withAdditionalProperty("ods:topicDiscipline", "botany"))
			.withSchemaDescription("A fancy mas making all dreams come true")
			.withSchemaCreativeWorkStatus("Definitely production ready")
			.withSchemaCodeRepository("https://github.com/DiSSCo/fancy-mas")
			.withSchemaProgrammingLanguage("Python")
	...
	public static MachineAnnotationService givenTombstoneMas() {
		return givenMas().withOdsStatus(MachineAnnotationService.OdsStatus.TOMBSTONE)
			.withSchemaVersion(2)
			.withSchemaDateModified(Date.from(UPDATED))
	...
	public static Map<String, Object> givenMasHandleRequestMap() {
		return Map.of("data", Map.of("type", "https://doi.org/21.T11148/a369e128df5ef31044d4", "attributes",
				Map.of("machineAnnotationServiceName", "A Machine Annotation Service")));
	}
	...
	public static Agent givenAgent() {
		return AgentUtils.createAgent(null, ORCID, AgentRoleType.CREATOR, "orcid", Type.SCHEMA_PERSON);
	}

	// Token
	public static Map<String, Object> givenClaims() {
		return Map.of("orcid", BARE_ORCID);
	}

	public static MasScheduleData givenMasScheduleData() {
		return new MasScheduleData(true, Set.of(HANDLE_ALT), Set.of(HANDLE_ALT));
