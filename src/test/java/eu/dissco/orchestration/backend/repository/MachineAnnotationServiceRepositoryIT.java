...
class MachineAnnotationServiceRepositoryIT extends BaseRepositoryIT {
	...
	void testGetMass() {
		...
		int pageNum = 1;
		int pageSize = 10;
		var machineAnnotationServices = IntStream.range(0, pageSize).boxed().map(this::givenMasWithId).toList();
	...
	void testGetMassSecondPage() {
		...
		int pageNum = 2;
		int pageSize = 10;
		var machineAnnotationServices = IntStream.range(0, pageSize + 1).boxed().map(this::givenMasWithId).toList();
		postMass(machineAnnotationServices);
	...
	void testCreateMasNullSchemaMaintainer() {
		// Given
		var mas = givenMas().withSchemaMaintainer(null);
