...
class SourceSystemRepositoryIT extends BaseRepositoryIT {
	...
	void testUniqueConstraintSourceSystem() {
		...
		// When
		assertThrows(IntegrityConstraintViolationException.class, () -> repository.createSourceSystem(sourceSystem));
	}
	...
	void testGetSourceSystems() {
		...
		int pageNum = 1;
		int pageSize = 10;
		List<SourceSystem> sourceSystems = IntStream.range(0, pageSize)
			.boxed()
			.map(this::givenSourceSystemWithId)
			.toList();
	...
	void testGetSourceSystemsSecondPage() {
		...
		List<SourceSystem> sourceSystems = IntStream.range(0, pageSize + 1)
			.boxed()
			.map(this::givenSourceSystemWithId)
			.toList();
		postSourceSystem(sourceSystems);
	...
	private SourceSystem givenSourceSystemWithId(int id) {
		return givenSourceSystem(String.valueOf(id), 1, OdsTranslatorType.DWCA, OBJECT_NAME + id, SS_ENDPOINT + id);
	}

	private List<SourceSystem> getAllSourceSystems() {
		return context.select(SOURCE_SYSTEM.DATA).from(SOURCE_SYSTEM).fetch(this::mapToSourceSystem);
	}
