...
public class SourceSystemController {
	...
	@Operation(summary = "Create a source system", description = """
			Create a source system from which to ingest data.
			User must have orchestration admin rights.
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "201", description = "Source system successfully created",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = SourceSystemResponseSingle.class)) }) })
	...
	public ResponseEntity<JsonApiWrapper> createSourceSystem(
			@io.swagger.v3.oas.annotations.parameters.RequestBody(
					...
					content = @Content(mediaType = "application/json",
							schema = @Schema(
									implementation = SourceSystemRequestSchema.class))) Authentication authentication,
		...
		log.info("Received create request for source system: {} from agent: {}", sourceSystemRequest, agent.getId());
		var result = service.createSourceSystem(sourceSystemRequest, agent, path);
		return ResponseEntity.status(HttpStatus.CREATED).body(result);
	}

	@Operation(summary = "Update a source system", description = """
			Update an existing source system.
			User must have orchestration admin rights.
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Source system successfully updated",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = SourceSystemResponseSingle.class)) }) })
	...
	public ResponseEntity<JsonApiWrapper> updateSourceSystem(Authentication authentication,
			...
			@Parameter(description = "Trigger ingestion immediately") @RequestParam(name = "trigger",
					defaultValue = "false") boolean trigger,
			@RequestBody SourceSystemRequestSchema requestBody, HttpServletRequest servletRequest)
			throws NotFoundException, ProcessingFailedException, ForbiddenException {
		var sourceSystemRequest = requestBody.data().attributes();
	...
	@Operation(summary = "Tombstone a source system", description = """
	...
	@Operation(summary = "Retrieve a source system", description = """
			Retrieve an existing source system, based on ID.
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Source system successfully retrieved",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = SourceSystemResponseSingle.class)) }) })
	@ResponseStatus(HttpStatus.OK)
	...
	@Operation(summary = "Retrieve an export of data a given source system", description = """
	...
	public ResponseEntity<Resource> getSourceSystemDownload(
			@Parameter(description = PREFIX_OAS) @PathVariable("prefix") String prefix,
			@Parameter(description = SUFFIX_OAS) @PathVariable("suffix") String suffix,
			@PathVariable("export-type") ExportType exportType) throws URISyntaxException, NotFoundException {
		var id = prefix + '/' + suffix;
		...
		return ResponseEntity.ok()
			...
			.header("Content-Disposition", "attachment; filename=\"" + generateDownloadName(id, exportType) + "\"")
			.body(resource);
	}

	@Operation(summary = "Run a source system ingestion", description = """
	...
	@Operation(summary = "Retrieve source systems", description = """
			Retrieve a paginated list of source systems.
			""")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "Source systems successfully retrieved",
			content = { @Content(mediaType = "application/json",
					schema = @Schema(implementation = SourceSystemResponseList.class)) }) })
	@ResponseStatus(HttpStatus.OK)
	@GetMapping("")
