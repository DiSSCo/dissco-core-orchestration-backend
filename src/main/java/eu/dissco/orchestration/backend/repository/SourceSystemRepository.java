...
public class SourceSystemRepository {
  ...
  public int updateSourceSystem(SourceSystem sourceSystem) {
    return context.update(NEW_SOURCE_SYSTEM)
        ...
        .set(NEW_SOURCE_SYSTEM.TRANSLATOR_TYPE, TranslatorType.valueOf(
            sourceSystem.getOdsTranslatorType().value()))
        .set(NEW_SOURCE_SYSTEM.DATA, mapToJSONB(sourceSystem))
        .where(NEW_SOURCE_SYSTEM.ID.eq(removeProxy(sourceSystem.getId()))).execute();
  }

  public SourceSystem getSourceSystem(String id) {
    return context.select(NEW_SOURCE_SYSTEM.DATA)
        .from(NEW_SOURCE_SYSTEM)
        .where(NEW_SOURCE_SYSTEM.ID.eq(removeProxy(id)))
  ...
  public Optional<SourceSystem> getActiveSourceSystem(String id) {
    return context.select(NEW_SOURCE_SYSTEM.DATA)
        .from(NEW_SOURCE_SYSTEM)
        .where(NEW_SOURCE_SYSTEM.ID.eq(removeProxy(id)))
        .and(NEW_SOURCE_SYSTEM.DATE_TOMBSTONED.isNull())
  ...
  public void deleteSourceSystem(String id, Date deleted) {
    context.update(NEW_SOURCE_SYSTEM)
        .set(NEW_SOURCE_SYSTEM.DATE_TOMBSTONED, deleted.toInstant())
        .where(NEW_SOURCE_SYSTEM.ID.eq(removeProxy(id)))
        .execute();
  }
  ...
  public List<SourceSystem> getSourceSystems(int pageNum, int pageSize) {
    ...
    return context.select(NEW_SOURCE_SYSTEM.DATA)
        .from(NEW_SOURCE_SYSTEM)
        .where(NEW_SOURCE_SYSTEM.DATE_TOMBSTONED.isNull())
        .limit(pageSize + 1)
        .offset(offset)
  ...
  public void rollbackSourceSystemCreation(String id) {
    context.deleteFrom(NEW_SOURCE_SYSTEM)
        .where(NEW_SOURCE_SYSTEM.ID.eq(removeProxy(id)))
        .execute();
  }
}
